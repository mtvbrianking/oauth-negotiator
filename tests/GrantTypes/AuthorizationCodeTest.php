...
use Bmatovu\OAuthNegotiator\GrantTypes\AuthorizationCode;
...
class AuthorizationCodeTest extends TestCase
{
    ...
    public function throws_exception_for_missing_args()
    {
        ...
        $missing = ['token_uri', 'client_id', 'client_secret', 'code'];

        $message = 'Parameters: '.implode(', ', $missing).' are required.';

        $this->expectExceptionMessage($message);

        new AuthorizationCode(new Client(), []);
    }
    ...
    public function can_obtain_new_access_token()
    {
        ...
        $grantType = new AuthorizationCode($client, [
            'token_uri'     => 'oauth/token',
            'client_id'     => $client_id,
            'client_secret' => $client_secret,
            'code'      => 'VPjqxtieLi6GiEiPxFeqZ5YGU',
        ...
        $this->assertEquals('authorization_code', $api_request_body['grant_type']);
        $this->assertEquals('VPjqxtieLi6GiEiPxFeqZ5YGU', $api_request_body['code']);
