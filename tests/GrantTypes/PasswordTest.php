...
class PasswordTest extends TestCase
{
    ...
    public function throws_exception_for_missing_args()
    {
        ...
        $missing = ['token_uri', 'client_id', 'client_secret', 'username', 'password'];

        $message = 'Parameters: '.implode(', ', $missing).' are required.';

        $this->expectExceptionMessage($message);

        new Password(new Client(), []);
    }
    ...
    public function can_obtain_new_access_token()
    {
        ...
        $grantType = new Password($client, [
            'token_uri'     => 'oauth/token',
            'client_id'     => $client_id,
            'client_secret' => $client_secret,
            'username'      => 'jdoe',
            'password'      => 'Some@random123.pswd',
        ...
        $this->assertEquals('password', $api_request_body['grant_type']);
        $this->assertEquals('jdoe', $api_request_body['username']);
        $this->assertEquals('Some@random123.pswd', $api_request_body['password']);
